...
const PrincipleCard: FC<PrincipleCardProps> = ({
  ...
  return (
    <div className="relative h-full group">
      ...
      <div className="absolute -top-6 right-4 select-none pointer-events-none text-[5.5rem] sm:text-[6.5rem] lg:text-[8rem] font-extrabold text-(--text-heading) opacity-[0.07] leading-none">
      ...
      <article
        className={`relative overflow-hidden h-full rounded-lg border border-gray-700 bg-[linear-gradient(145deg,var(--bg-secondary),var(--bg-primary))] transition-transform duration-200 ease-out transform hover:-translate-y-2 hover:scale-[1.01] ${
          featured
            ? "ring-4 ring-(--text-accent) shadow-[0_10px_30px_rgba(255,254,0,0.12)]"
            : "hover:ring-4 hover:ring-(--text-accent)"
        }`}
      >
        ...
        <div className="p-6">
          ...
          {lead && (
            <p className="text-(--text-subheading) font-semibold mb-1">
              {lead}
            </p>
          )}
          ...
          <div className="mt-6">
            <div className="h-0.5 w-12 bg-(--text-accent) rounded-full opacity-60" />
          </div>
        </div>
      </article>
    </div>
